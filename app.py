...
# --- Unduh resource NLTK (hanya pertama kali) ---
nltk.download('punkt')
...
# --- Streamlit UI ---
st.title("🔍 Pencarian Artikel Berita")
...
if query:
    ...
    if results:
        st.success(f"Ditemukan {len(results)} artikel relevan:")
        for res in results:
            st.markdown(f"### {res['title']}")
            st.markdown(f"**Domain:** {res['domain']}  |  **Tanggal:** {res['publish_date']}  |  **Skor:** {res['score']:.4f}")
            st.markdown(f"{res['snippet']}...")
            st.markdown("---")
    else:
        st.warning("Tidak ada artikel relevan ditemukan.")
